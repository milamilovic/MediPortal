...
namespace ZdravoCorp.HealthInstitution.GUI.Equipment.Controllers
{
    internal class EquipmentFilterSearchController
    {
        public static TableRow MakeRowForEquipmentDisplay(EquipmentStorageItem item)
        {
            ...
            cell = new TableCell();
            bool isDynamic = EquipmentService.IsEquipmentDinamicByName(item.StoredEquipmentName);
            if (isDynamic)
            {
            ...
            cell = new TableCell();
            cell.Blocks.Add(new Paragraph(new Run(EquipmentService.GetEquipmentCategoryByName(item.StoredEquipmentName).ToString())));
            row.Cells.Add(cell);
